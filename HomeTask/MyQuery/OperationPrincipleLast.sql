...
+ CASE WHEN len(AD.[CityRegionName]) > 0 THEN  IsNull(', ' + LTrim(RTrim(AD.[CityRegionName])) + ' район ','') 
ELSE ''END 
+ IsNull(', ' + LTrim(RTrim(IsNull(AD.StreetTypeShortName,AD.[StreetTypeName]))) + ' ' + LTrim(RTrim(AD.[StreetName])),'') +
          (Case Len(isnull(adr.Building, '')) When 0 Then '' Else ', буд.'+LTRIM(RTRIM(Adr.Building)) End)+
          (Case Len(isnull(Adr.BuildingPart, '')) When 0 Then '' Else ', корп.'+LTRIM(RTRIM(Adr.BuildingPart)) End)+
          (Case Len(isnull(Adr.Apartment, '')) When 0 Then '' Else ', кв.'+LTRIM(RTRIM(Adr.Apartment)) End)  as AddressPerson,
	oe.FullPathName,
	c.CounterNumber,
	(CASE 
		WHEN c.Phase= 0	THEN 3
				...
				END) AS Phase,
				(CASE WHEN C.OperatingPrinciple=0 	THEN 'Індукційний'
							WHEN c.OperatingPrinciple=1 THEN 'Електронний'
...
FROM Measuring.CounterType AS CT
		...
		JOIN AccountingCommon.Point AS P ON C.PointId = P.PointId
		JOIN AccountingDictionary.OblElement AS OE ON P.OblElementId = OE.OblElementId
		JOIN AccountingCommon.UsageObject AS UO ON P.UsageObjectId = UO.UsageObjectId
		join AccountingCommon.Address Adr ON uo.AddressId = Adr.AddressId
		JOIN AccountingCommon.Account AS A ON UO.AccountId = A.AccountId 
		JOIN AccountingCommon.PhysicalPerson AS PP ON  A.PhysicalPersonId = PP.PhysicalPersonId
 ...
 WHERE 
 C.DateTo='20790606'
AND ct.Code IN (10374,10377,10378,10381,10386,10388,10392,10393,
10394,10396,10400,10402,10407,10409,10412,10426,10428,10429,10430,
10458,10460,10461,10463,10465,10466,10467,10468,
10470,10471,10472,10477,10481
)
ORDER BY c.DateFrom

...
WHERE [GI].[PlacementLocationId]=[PL].[PlacementLocationId] AND [GI].[Date]=[VC].[IndexDate] AND [C].[CoreEquipmentId]=[CE].[CoreEquipmentId]  )
 ...
 --признак "не для розрахунку" для показників лічильників, на дату яких вже є показник в базі 
--UPDATE igi
--			SET [IsForCalculate] = 0
--FROM Counter.GroupIndex AS iGI
--WHERE EXISTS 
--(SELECT * FROM Counter.GroupIndex AS GI
--	JOIN  DirectorateServer.PRDIR_Equipment.SmartIms.v_CounterDataJur AS   VCD  ON GI.CounterMeasuringId = VCD.CounterMeasuringId
--where   igi.[GroupIndexSourceId]<>104 AND [iGI].[Date]=[VCD].IndexDate 
--	AND VCD.CounterMeasuringId=iGI.CounterMeasuringId )
...
FROM DirectorateServer.PRDIR_Equipment.SmartIms.v_CounterDataJur AS VCDJ
...
WHERE GI.IsForCalculate=0
	...
	AND NOT EXISTS
	(SELECT * FROM DirectorateServer.PRDIR_Equipment.SmartIms.v_CounterDataJur AS VCDJ2
		...
		(select * from DirectorateServer.PRDIR_Equipment.SmartIms.v_CounterDataJur AS VCDJ2
			...
			join [JuridicalDB].dbo.counterMeterage as CMJ on CMJ.CounterId=c.CounterId
			 where  CMJ.EquipmentGroupIndexGuid=GI.guid
                                                        ...
                                                        '
SET @sql = [Services].[ReplaceDatabasePath]('JuridicalDB', @sql)
